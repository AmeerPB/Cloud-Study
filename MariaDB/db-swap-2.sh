...
# Step 10: Wait for the volume to detach
...
# Step 11: Delete the detached root volume

echo "Deleting the detached root volume $CURRENT_ROOT_VOLUME_ID..."

aws ec2 delete-volume --volume-id $CURRENT_ROOT_VOLUME_ID --region $REGION
 
if [ $? -ne 0 ]; then

    echo "Failed to delete the detached root volume $CURRENT_ROOT_VOLUME_ID."

    exit 1

fi
 
echo "Detached root volume $CURRENT_ROOT_VOLUME_ID deleted successfully."
 
# Step 12: Attach the new volume to instance-2 as the root volume
...
# step  13: Wait for the volume to attach
...
# Step 16: Backup the MySQL config file


ssh -i $SSH_KEY_LOCATION $SERVER_USERNAME@$REPORT_SERVER_IP -- sudo mv /etc/mysql/mariadb.conf/50-server.cnf /etc/mysql/mariadb.conf/50-server.cnf.backup.$(date +%d%b%Y-%H%M)
ssh -i $SSH_KEY_LOCATION $SERVER_USERNAME@$REPORT_SERVER_IP -- sudo mv /etc/mysql/mariadb.conf/50-server.cnf.bak /etc/mysql/mariadb.conf/50-server.cnf
 

# Step 17: Enable MySQL service
echo "Enabling MySQL service..."
ssh -i $SSH_KEY_LOCATION $SERVER_USERNAME@$REPORT_SERVER_IP -- sudo systemctl enable mysql



# Step 18: Restart Instance-2
ssh -i $SSH_KEY_LOCATION $SERVER_USERNAME@$REPORT_SERVER_IP -- sudo reboot


# # Step XX: Start MySQL service
# echo "Starting MySQL service..."
# ssh -i SSH_KEY_LOCATION SERVER_USERNAME@REPORT_SERVER_IP -- sudo systemctl start mysql

 

# Step XX: Wait for MySQL service to be active
